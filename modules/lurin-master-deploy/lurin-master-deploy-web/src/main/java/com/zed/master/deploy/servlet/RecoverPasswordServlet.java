...
public class RecoverPasswordServlet extends HttpServlet {
     ...
     * Logger Instance
     */
    private static Logger LOGGER = Logger.getLogger(RecoverPasswordServlet.class.getName());
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        LOGGER.debug("Access recover password Servlet");
        ...
        try {
            ...
            LOGGER.debug(email);
            String response = this.usersServices.sendRecoverPasswordMail(email);
            LOGGER.debug(response);
            //PrintWriter writer = resp.getWriter();
            //writer.append(response);
 	        req.setAttribute("code", response);
 	        req.getRequestDispatcher("retrievepassword.jsp").forward(req, resp);
            //writeResponse(resp, response);
 	   } catch(Exception ex) {
 		   ex.printStackTrace();
 		  //writeResponse(resp, ex.getMessage());
 		   req.setAttribute("code", "002");
	       req.getRequestDispatcher("retrievepassword.jsp").forward(req, resp);
 	   }
